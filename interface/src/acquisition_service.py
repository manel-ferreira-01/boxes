...
def handle_input(image: Image.Image, session_id: str):
    ...
    if image is not None:
        try:
            ...
            logging.info(f"Gradio: Put image (bytes: {len(image_bytes)}) with label to acquisition queue.")
...
#    if not data_display_queue.empty():
    if True :
        try:
            ...
            logging.info(f"Gradio display_img: Displaying image (shape: {img_np.shape}) with info: {info}")
